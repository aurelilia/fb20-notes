...
## Äquivalenzautomat
$L$-Äquivalenzklassenautomat ist bis auf Isomorphie der einzige DFA, der $L$ mit $index(\sim L)$-vielen,
d.h. mit minimal vielen, Zuständen erkennt
...
Jeder DFA mit minimal vielen Zuständen, der $L$ erkennt, ist zum Äquivalenzklassen-Automat $\sim_L$ isomorph.
...
sich jedes $x \in L$ mit $|x| \ge n$ zerlegen lässt in $x = uvw, v \not= \epsilon, |uv| \le n$ und für alle $m \in \mathbb{N}$:  
$u \cdot v^m \cdot w = u \cdot v \cdot\cdot\cdot v \cdot w \in L$ ($v \cdot\cdot\cdot v$: $m$ mal)
